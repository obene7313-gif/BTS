...
# --- DOSYADAN VERİ ÇEKME (TAM YÜKLEME) ---
try:
...
bts_sorulari = [
    {"soru": "BTS hangi yılda çıkış yapmıştır?", "cevap": "2013", "siklar": ["2011", "2012", "2013", "2014"]},
    ...
    {"soru": "BTS hangi şirketlerin çatısı altında kurulmuştur?", "cevap": "Big Hit (HYBE)", "siklar": ["SM", "YG", "JYP", "Big Hit (HYBE)"]},
    ...
    {"soru": "Hangi BTS üyesi modern dans eğitimi almıştır ve Busan Sanat Lisesi'ne birincilikle girmiştir?", "cevap": "Jimin", "siklar": ["J-Hope", "Jimin", "V", "Jungkook"]},
    {"soru": "BTS'in 'Love Yourself' albüm serisinin başlık şarkısı hangisidir?", "cevap": "Fake Love", "siklar": ["DNA", "Fake Love", "Idol", "Run"]},
    {"soru": "Hangi üye grubun 'Golden Maknae'si (Altın Maknae) olarak bilinir?", "cevap": "Jungkook", "siklar": ["Jimin", "V", "Jungkook", "Jin"]},
    {"soru": "BTS'in hayranlarına özel tasarladığı resmi ışık çubuğun (lightstick) adı nedir?", "cevap": "Army Bomb", "siklar": ["Muster Stick", "Army Bomb", "Bangtan Light", "Purple Rod"]},
    {"soru": "BTS, Birleşmiş Milletler (UN) genel kurulunda ilk kez hangi yılda konuşma yapmıştır?", "cevap": "2018", "siklar": ["2016", "2017", "2018", "2019"]},
    {"soru": "BTS'in popüler reality show programının adı nedir?", "cevap": "Run BTS!", "siklar": ["BTS In The Soop", "Run BTS!", "Rookie King", "American Hustle Life"]},
    ...
    {"soru": "BTS'in Line Friends ile iş birliği yaparak oluşturduğu karakter serisinin adı nedir?", "cevap": "BT21", "siklar": ["BTS-Toons", "BT21", "Bangtan Pets", "LINE-BTS"]},
    ...
    {"soru": "BTS'in hangi albümle onlara ilk kez bir Daesang (Yılın Albümü) kazandırmıştır?", "cevap": "The Most Beautiful Moment in Life: Young Forever", "siklar": ["Wings", "Dark & Wild", "The Most Beautiful Moment in Life: Young Forever", "Love Yourself: Tear"]},
    ...
    {"soru": "BTS'in Haklar Kurumu UNICEF ile birlikte yürüttüğü kampanyanın adı nedir?", "cevap": "Love Myself", "siklar": ["Save Me", "Love Myself", "End Violence", "Be Yourself"]},
    {"soru": "J-Hope'un Becky G ile iş birliği yaptığı solo hit şarkısı hangisidir?", "cevap": "Chicken Noodle Soup", "siklar": ["More", "Arson", "Daydream", "Chicken Noodle Soup"]},
    ...
    {"soru": "BTS'in 2016 yılında yayınlanan ve 'Kan, ter ve gözyaşı' sözleriyle bilinen ünlü şarkısı hangisidir?", "cevap": "Blood Sweat & Tears", "siklar": ["Wings", "Blood Sweat & Tears", "Save Me", "Fire"]},
    ...
    {"soru": "BTS'in 'Black Swan' şarkısının ilk koreografi videosunda hangi tarz dans ön plana çıkmaktadır?", "cevap": "Modern Bale / Çağdaş Dans", "siklar": ["Hip-hop", "Breakdance", "Modern Bale / Çağdaş Dans", "Poping"]},
    ...
    {"soru": "Hangi BTS klibi tren garı, lunapark ve kar temalı sahneler içerir, derin dostluğu anlatır?", "cevap": "Spring Day", "siklar": ["Spring Day", "Run", "I Need U", "Life Goes On"]},
    ...
    {"soru": "Jimin'in BT21 karakteri olan sarı kapüşonlu köpekçiğin adı nedir?", "cevap": "Chimmy", "siklar": ["Chimmy", "Cooky", "Shooky", "Van"]}
]
...
class GameView(View):
    ...
    def make_callback(self, sik):
        async def callback(interaction: discord.Interaction):
            if self.cevaplandi:
                await interaction.response.send_message("⚠️ Bu soru zaten cevaplandı!", ephemeral=True)
                return
...
async def on_message(message):
    ...
    for mention in message.mentions:
        if mention.id in afk_users:
            ...
            await message.channel.send(f"👤 {message.author.mention}, etiketlediğin kullanıcı **{mention.name}** AFK!\n**Sebep:** {sebep}")
    ...
    # 2. %2 BTS Sorusu
        ...
        msg = await message.channel.send(f"🎯 **BTS TRIVIA SORUSU!**\n\n**{soru_data['soru']}**\nDoğru ÂŞıkkı İŞaretle! (Süre: 15sn)*", view=view)

        await asyncio.sleep(15)
        if not view.cevaplandi:
            try:
                await msg.edit(content=f"⏰ Süre doldu! Doğru cevap **{soru_data['cevap']}** olacaktı.", view=None)
            except:
                pass
        return
    ...
    # 3. %7 Matematik Sorusu
        ...
        if not view.cevaplandi:
            try:
                await msg.edit(content=f"⏰ Süre doldu! Doğru cevap **{cevap}** olacaktı.", view=None)
            except:
                pass
        return
...
async def ayarlar(ctx):
    ...
    embed.add_field(name="Log Kanalı", value=f"<#{server_settings['log_kanal']}>" if server_settings['log_kanal'] else "❌ Ayarlanmadı")
    embed.add_field(name="Giriş-Çıkış Kanalı", value=f"<#{server_settings['welcome_kanal']}>" if server_settings['welcome_kanal'] else "❌ Ayarlanmadı")
...
async def karaliste(ctx, islem=None, *, kelime=None):
    if not islem:
        await ctx.send(f"⚙️ **Yasaklı Kelimeler:** {', '.join(server_settings['karaliste'])}")
...
async def sustur(ctx, member: discord.Member, sure: str, *, sebep="Belirtilmedi"):
    ...
    if unit not in time_dict or not sure[:-1].isdigit():
        await ctx.send("⚠️ Geçersiz süre formatı! Örn: 5m, 2h, 1d")
        return
...
async def nuke(ctx):
    ...
    await new_ch.send("💥 Kanal başarıyla sıfırlandı (Nuke)!\nhttps://tenor.com/view/explosion-mushroom-cloud-atomic-bomb-bomb-gif-4464835")
...
async def ban(ctx, member: discord.Member, *, sebep="Belirtilmedi"):
    ...
    await ctx.send(f"✈️ **{member.name}** sunucudan banlandı. Sebep: {sebep}")
...
async def lock(ctx):
    ...
    await ctx.send("🔒 Kanal yazmaya kapatıldı!")
...
async def unlock(ctx):
    ...
    await ctx.send("🔓 Kanal tekrar yazmaya açıldı!")

# --- AÇIKLAMALI AFK SİSTEMİ ---
@bot.command()
async def afk(ctx, *, sebep=None):
    ...
    if ctx.author.id in afk_users:
        ...
        embed.add_field(name="⏱️ AFK Süresi", value=", ".join(sure) if sure else "1 saniyeden az", inline=False)
        await ctx.send(embed=embed)
        return
...
async def ucanguvercin(ctx, member: discord.Member):
    await ctx.send(f"🕊️💥 {ctx.author.mention}, {member.mention} kullanıcısına uçarak gelen bir uçan güvercin fırlattı!\n**Tekme atıyor bu güvercin sana!**\nhttps://tenor.com/view/pigeon-kick-funny-birds-gif-14470635")
...
async def slaps(ctx, member: discord.Member):
    await ctx.send(f"🖐️💥 {ctx.author.mention}, {member.mention} kullanıcısına Osmanlı tokatıyla vurdu!\nhttps://tenor.com/view/slap-in-the-face-angry-gif-14689404")

@bot.command()
async def kiss(ctx, member: discord.Member):
    await ctx.send(f"😘 {ctx.author.mention}, {member.mention} kullanıcısına sulu sulu öptü!\nhttps://tenor.com/view/anime-kiss-gif-25745155")
...
async def op(ctx, member: discord.Member):
    await ctx.send(f"😘 {ctx.author.mention}, {member.mention} kullanıcısını öptü.")
...
async def efkarolcer(ctx):
    oran = random.randint(0, 100)
    await ctx.send(f"🚬 {ctx.author.mention} bugün efkar durumun: **%{oran}**")

@bot.command()
async def sansolcer(ctx):
    oran = random.randint(0, 100)
    await ctx.send(f"🎲 {ctx.author.mention} bugün şans durumun: **%{oran}**")
...
class BTSSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        embed.set_footer(text="💜 BTS Bilgi Menüsü")
...
async def bts(ctx):
    embed = discord.Embed(
        ...
        description="Aşağıdaki menüden bir BTS üyesi seç.",
...
async def slots(ctx, miktar: int):
    ...
    if miktar <= 0 or miktar > bakiye:
        await ctx.send("⚠️ Geçersiz miktar veya yetersiz bakiye!")
        return
...
# --- BİLGİ & SİSTEM ---
@bot.command()
async def sping(ctx, member: discord.Member = None):
    target = member or ctx.author
    spotify_act = None
    ...
    if spotify_act:
        ...
        await ctx.send(f"ℹ️ {target.mention} şu an Spotify'da bir şey dinlemiyor veya durumu kapalı.")
...
async def kullanici(ctx, member: discord.Member = None):
    ...
    embed.add_field(name="Sunucuya Katılma", value=target.joined_at.strftime('%d/%m/%Y') if target.joined_at else "Bilinmiyor", inline=True)
    await ctx.send(embed=embed)
...
async def yardim(ctx):
    ...
    embed.add_field(name="🎭 Eğlence & Etkileşim", value="afk, ucanguvercin, saat, slaps, kiss, saril, askolcer, efkarolcer, sanslisayi, ship, ship2", inline=False)
    embed.add_field(name="💎 Ekonomi & Sistem", value="para, slots, sping, kullanici, sunucu", inline=False)
    await ctx.send(embed=embed)
